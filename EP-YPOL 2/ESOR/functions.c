...
void print_matrix_1d(double * L, int size_a){
...
void print_matrix_2d(double ** L, int size_a){
    for(int i=0; i<size_a; i++){
...
void fill_matrix_b(double *arr, int n, int option, char c, char *filename){
    ...
    switch(option){
        ...
        case 2:
            ...
            for(  i = 0; i < n; i++){
                arr[i] = (double)((rand()%(upper-lower+1))+lower);
            }
            break;
        case 3:
            if( (fp = fopen(filename, "r"))==NULL ){
