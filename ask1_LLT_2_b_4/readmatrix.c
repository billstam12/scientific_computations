...
void read_user_input(long double **arr, int n, int m, char c){
...
void create_random(long double **arr, int n, int m, char c){
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            arr[i][j] = (long double)((rand()%(upper-lower+1))+lower);
        }
    }
}
...
void read_matrix_from_file(long double **arr, int n, int m, char c, char *filename){
    FILE *fp;
    if( (fp = fopen(filename, "r"))==NULL ){
